...
test('wrong browser origin is rejected', async () => {
  ...
  try {
    const response = await onRequestPost({
      request: requestFrom({ name: 'Guest', phone: '2394102846', message: 'Hello', privacy_consent: 'true' }, 'https://unapproved.test'),
      env: baseEnv,
    });
...
test('failed Turnstile verification blocks delivery', async () => {
  ...
  try {
    const response = await onRequestPost({
      request: requestFrom({
        name: 'Guest', phone: '2394102846', message: 'Hello', privacy_consent: 'true', 'cf-turnstile-response': 'bad-token',
      }),
      env: baseEnv,
    });
    assert.equal(response.status, 400);
...
test('Turnstile hostname mismatch blocks delivery', async () => {
  ...
  try {
    const response = await onRequestPost({
      request: requestFrom({
        name: 'Guest', phone: '2394102846', message: 'Hello', privacy_consent: 'true', 'cf-turnstile-response': 'valid-token',
      }),
      env: baseEnv,
    });
    assert.equal(response.status, 400);
