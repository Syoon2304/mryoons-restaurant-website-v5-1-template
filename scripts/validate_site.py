...
REQUIRED_MANIFEST_KEYS = {
    ...
    "primary_action", "pages", "external_media", "public_documents", "forms",
}
...
def validate_manifest(
    ...
    if schema_path is None or not schema_path.is_file():
        report.error("manifest.schema_missing", "The protected V5.1 manifest schema is required.", schema_path or "infrastructure/site-manifest.schema.json")
    else:
        try:
    ...
    if not isinstance(pages, list) or not pages:
        ...
        for idx, page in enumerate(pages):
            if not isinstance(page, dict) or not isinstance(page.get("path"), str):
                report.error("manifest.page_entry", f"pages[{idx}] must contain a string path.", "site-manifest.json")
                continue
            try:
                rel = normalize_page_path(page["path"])
            except ValueError as exc:
                report.error("manifest.page_path", f"pages[{idx}] has an invalid path: {exc}", "site-manifest.json")
                continue
            ...
            if rel in normalized:
                report.error("manifest.page_duplicate", f"Manifest page path is duplicated: {rel}", "site-manifest.json")
            normalized.add(rel)
    ...
    for key in ("external_media", "public_documents", "forms"):
            ...
            if token.startswith(("https://", "http://")):
                ...
                if host is None:
                    ...
                    report.error(
                        ...
                        f"Content-Security-Policy {directive} allows {host}, but the V5.1 manifest does not declare a feature that needs it.",
                        "_headers",
                    )
...
def validate_manifest_page_titles(
    ...
    for index, item in enumerate(manifest["pages"]):
        if not isinstance(item, dict) or not isinstance(item.get("path"), str):
            continue
        try:
            rel = normalize_page_path(item["path"])
        except ValueError:
            continue
...
def validate_page(
    ...
    if production:
        if not p.canonical:
            ...
            expected = domain + ("/" if rel == "index.html" else "/" + rel)
...
def validate_links(root: Path, pages: list[ParsedPage], report: ValidationReport) -> None:
        ...
        for _, raw in pattern.findall(text):
            ...
            if not url or url.startswith(("data:", "http://", "https://", "#")):
                if url.startswith("http://"):
                    ...
                    report.error("css.external_asset", "CSS-referenced public assets must be packaged locally under the V5.1 CSP.", rel)
                continue
            ...
            if tag == "img":
                ...
                for candidate in candidates:
                    if candidate.startswith(("http://", "https://")):
                        report.error("asset.external_image", "Public images must be optimized local files under the V5.1 CSP.", page.relpath)
...
def validate_site(
    ...
    validate_links(root, pages, report)
