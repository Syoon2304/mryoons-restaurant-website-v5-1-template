# Codex operating instructions — Restaurant Website V5.1
...
5. The connected V5.1 Restaurant Website Starter Pack and private business-assets source, when available in the current context.
...
- `public/`;
- the three `handoff/` files, but only when the owner has approved the public facts or decisions being recorded.
...
For every production change, keep `site-manifest.json` and `version.json` synchronized. Schema version is `2.0`, workflow version is `5.1`, and repository package spec is `5.1`. Manifest page paths are canonical relative file paths without a leading slash.
